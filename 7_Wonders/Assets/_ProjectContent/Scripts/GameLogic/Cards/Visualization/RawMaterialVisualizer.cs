...
namespace WhiteTeam.GameLogic.Cards.Visualization
{
    public class RawMaterialVisualizer : CardVisualizer<RawMaterialsCard>
    {
        ...
        {
            return UnityEngine.Resources.Load<Sprite>("Pictures/raw_material_symbol");
        }
        ...
        public override Sprite GetCurrentEffect()
        {
            ...
            if (currencyItems.Length == 1)
            {
                foreach (var action in currencyItems)
                {
                    ...
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/wood_wood");
                    }

                    //STONE 1
                    if (action.Currency == Resources.Resource.CurrencyProducts.STONE && action.Amount == 1)
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/stone");
                    }

                    //STONE 2
                    if (action.Currency == Resources.Resource.CurrencyProducts.STONE && action.Amount == 2)
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/stone_stone");
                    }

                    //CLAY 1
                    if (action.Currency == Resources.Resource.CurrencyProducts.CLAY && action.Amount == 1)
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/brick");
                    }

                    //CLAY 2
                    if (action.Currency == Resources.Resource.CurrencyProducts.CLAY && action.Amount == 2)
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/brick_brick");
                    }

                    //ORE 1
                    if (action.Currency == Resources.Resource.CurrencyProducts.ORE && action.Amount == 1)
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/Gold ore");
                    }

                    //ORE 2
                    if (action.Currency == Resources.Resource.CurrencyProducts.ORE && action.Amount == 2)
                    {
                        effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/ore_ore");
                    }
                }
            }
            ...
            if (currencyItems.Length == 2)
            {
                ...
                for (int i = 0; i < currencyItems.Length; i++)
                {
                        ...
                        currencyItems[i].Amount == 1)
                    {
                        ...
                        {
                            effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/wood_brick");
                        }
                    }
                        ...
                        currencyItems[i].Amount == 1)
                    {
                        if (currencyItems[i + 1].Currency == Resources.Resource.CurrencyProducts.CLAY &&
                            currencyItems[i + 1].Amount == 1)
                        {
                            effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/stone_brick");
                        }
                    }
                        ...
                        currencyItems[i].Amount == 1)
                    {
                        if (currencyItems[i + 1].Currency == Resources.Resource.CurrencyProducts.ORE &&
                            currencyItems[i + 1].Amount == 1)
                        {
                            effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/brick_ore");
                        }
                    }
                        ...
                        currencyItems[i].Amount == 1)
                    {
                        if (currencyItems[i + 1].Currency == Resources.Resource.CurrencyProducts.WOOD &&
                            currencyItems[i + 1].Amount == 1)
                        {
                            effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/stone_wood");
                        }
                    }
                        ...
                        currencyItems[i].Amount == 1)
                    {
                        if (currencyItems[i + 1].Currency == Resources.Resource.CurrencyProducts.ORE &&
                            currencyItems[i + 1].Amount == 1)
                        {
                            effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/wood_ore");
                        }
                    }
                        ...
                        currencyItems[i].Amount == 1)
                    {
                        if (currencyItems[i + 1].Currency == Resources.Resource.CurrencyProducts.STONE &&
                            currencyItems[i + 1].Amount == 1)
                        {
                            effectRawMaterials = UnityEngine.Resources.Load<Sprite>("Effects/ore_stone");
                        }
                    }
                }
            }
